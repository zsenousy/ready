...
def training_loop(model, current_idx, current_data, optimizer, training_performance_dict, loss_fn, cuda_available):
    ...
    batch_metrics = evaluate(output, labels, n_classes=4, _hausdorff=True) #skip hausdorff distance calculation during training to save time
...
def validation_loop(model, current_idx, current_data, optimizer, validation_performance_dict, loss_fn, cuda_available):
    ...
    batch_metrics = evaluate(output, labels, n_classes=4, _hausdorff=True) #skip hausdorff distance calculation during validation to save time
