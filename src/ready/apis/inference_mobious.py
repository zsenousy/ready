...
if __name__ == "__main__":
    ...
    if weighted_files:
        ...
        model.eval()
        data_root = "Scratch/scratch/ccaekqu/datasets/ready/ready/inference/inference_results/mobious"
    ...
    with torch.no_grad():
        ...
        #   print(f"outputs mean {torch.mean(outputs)}")  # outputs mean -4.6653242111206055
         ...
         #  )
            ...
            ax[0].set_ylabel("RAW IMAGE[400,640,3]")
         #  ax[0, 1].set_title("ch0")
          ...
          # ax[1, 4].imshow(label.squeeze(0).cpu() > 3)
            ax[1].set_ylabel("MASK IMAGE[400,640]")
